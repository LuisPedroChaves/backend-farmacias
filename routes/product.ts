...
PRODUCT_ROUTER.get("/", mdAuth, (req: Request, res: Response) => {
  ...
  Product.find(query)
    ...
    .exec(async (err, products) => {
      ...
      const TOTAL: number = await Product.find(query).countDocuments().exec();
  ...
  if (!PRODUCT) {
    if (BODY._brand) {
      ...
      if (_brand) {
        ...
        let product = await NEW_PRODUCT.save().then();
      } else {
        const NEW_PRODUCT = new Product({
          code: 0,
          barcode,
...
PRODUCT_ROUTER.put("/:id", mdAuth, (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "🚀 ~ file: product.ts ~ line 23 ~ PRODUCT_ROUTER.put ~ error",
      error
    );
  }
});
...
PRODUCT_ROUTER.post("/", mdAuth, (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "🚀 ~ file: product.ts ~ line 10 ~ PRODUCT_ROUTER.post ~ error",
      error
    );
  }
});
      ...
      try {
        if (code > 0) {
          ...
          if (_brand) {
            ...
            //           _sus = await SUBSTANCE.save().then();
            //         }
            ...
            const DESCRIPTION: string = doc[2];

            //   // PRESENTACION POR UNIDAD
            const PRESENTATIONS: any = [];
            const name: string = "UNIDAD";
            const cost: number = doc[4];
            ...
            const LAST_BRAND = await Brand.findOne(
              {
                deleted: false,
              },
              "code"
            ...
            const SAVE_BRAND = await NEW_BRAND.save().then();
        ...
        code++;
        console.log(
          "🚀 ~ file: product.ts ~ line 372 ~ awaitbluebird.mapSeries ~ code",
          code
        );
      } catch (e: any) {
...
PRODUCT_ROUTER.post("/xlsx/ticket", (req: Request, res: Response) => {
  ...
  FILE.mv(PATH, async (err: any) => {
    ...
    await bluebird.mapSeries(DOC[0].data, async (doc: any, index) => {
      try {
        const BARCODE: string = String(doc[0]).trim();
        let date = new Date(
          moment(ExcelDateToJSDate(doc[3])).tz("America/Guatemala").format()
        );
